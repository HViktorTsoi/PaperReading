...
7.  Grid Context Aggregation：每个点的特征由以下组成：
	...
	3. Edge attention,像pointnet++这类的网络没有考虑edge attention，如果pointnet2的group也看做一个图的话，那么就是中心点的所有edge连接的邻居的feature经过mlp之后，直接得到映射之后的feature，每个邻居(或者说edge)的权重是相等的；而edge attention则是给每一条edge再单独学出来一个权重，然后乘到mlp之后的feature上，从而体现出在图中不同邻居的重要性的不同。(这里有点像SEBlock，只不过后者是在通道)
