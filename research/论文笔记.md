...
7.  Grid Context Aggregation：每个点的特征由以下组成：
	...
	4. Aggregation，最终对于点i，它的特征学习对应的输入是它的feature vector；它的edge attention对应的输入是：group center的坐标，点i的坐标，点i的coverage Weight，点i的feature vector(文章中称之为semantic)，以及Grid Context Pooling(即pooling之后得到的grouping center的feature)。在学习到下一层的
